...
import type { Job, JobLog, FFmpegProgress, JobStatus } from '@/api/types'
...
export function useJobStream(id: string) {
	...
	api.jobs.get(id)
		.then(async (j) => {
			...
			if (initialLogs.length) nextSeq = initialLogs[initialLogs.length - 1]!.seq
			loaded.value = true
		})
		.catch((e) => {
	...
	const sse = streamEvents(`/api/jobs/${id}/events`, {
		status(ev) {
			const d = ev.data as { jobId: string; status: JobStatus; error?: string }
			if (job.value) {
				job.value.status = d.status
				if (d.error) job.value.error = d.error
			}
		},
		progress(ev) {
			if (job.value) job.value.progress = ev.data as FFmpegProgress
		},
		log(ev) {
			const log = ev.data as JobLog
